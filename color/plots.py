...
def custom_spectrum_plot(
    spectrum,
    title,
    filename,
    xlabel = 'Wavelength ($nm$)',
    ylabel = 'Intensity ($W/m^2$)',
    lab_color = colormodels.lab_color(0,0,0)):
    pylab.clf ()
    # upper plot - solid patch of color that matches the spectrum color
    pylab.subplot (2,1,1)
    pylab.title (title)
    color_string = colormodels.irgb_string_from_rgb(colormodels.rgb_from_xyz(colormodels.xyz_from_lab(lab_color)))
